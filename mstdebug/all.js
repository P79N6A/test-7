...
$(function() {
		...
		var $ = jQuery,
			...
			mstStaticHost = "http://mst.vipstatic.com";

		//::加载依赖的js 并执行对应的回调队列	
		VIP = win.VIP = win.VIP || {}, win.mstRequire = win.mstRequire || {
			path: {
				hercule: mstStaticHost + "/demo/js/hercules.js",
				brand: mstStaticHost + "/demo/js/modules/brand.js",
				product: mstStaticHost + "/demo/js/modules/product.js"
			}
		}, $.extend(win.mstRequire, {
			hercule: {//::加载脚本hercule.js并执行回调队列 下同
				callbackList: []
			},
			brand: {
				callbackList: []
		...
		$.extend(Renderer.prototype, {
				...
				initReload: function() {//设置页面定时重载
					...
					i.setHours(10),
					i = +i + 864e5, // 第二天 10:00
					// 当前时间 小于 早10 则 早10+5min-
					// 当前时间 大于 晚8 则 明早10+5min-
					// 当前时间 小于 晚8 则 晚8+5min-
					e = n < t ? t + o : n > a ? i + o : a + o;
					setInterval(function() {//::轮询当前时间是否到达目标时间点，是则重载页面
						var t = +new Date;
						t > e && location.reload()
					}, 1e3)
				},
				render: function() {
					var e = this,
						...
						a = !0,
						i = ["custom_config", "share"],
						// div.plugin-head-group
					...
					//遍历组件
					for (var o = t.length - 1; o >= 0 && !e.checkLastModule(t[o]); o--);
					
					$.each(t, function(t, o) {
						var r, s, d, c = a || document.documentElement.clientWidth <= 540 ? document.documentElement.clientWidth > 632 ? 632 : document.documentElement.clientWidth : 540,
							// div.plugin
							p = $("<div></div>").addClass("plugin"),
							// not in ['custom_config', 'share']
							l = i.indexOf(o.module_type_id) == -1,
							u = "share" == o.module_type_id ? "1" : o.extend && o.extend.grade || "3"; // u -> style's grade 

						//e.pluginList -> [div.plugin[data-index=n], ..]	
						//checkLiveTime 活动是否结束？
						if (e.pluginList.push(p.attr("data-index", t)), !e.checkLiveTime(o)) return !0;
						//图片等比缩放至与屏幕等宽
						switch (r = o.extend ? o.extend.layout || {} : {}, d = c / (r.width || 0), s = parseInt((r.height || 0) * d) || 0, o.module_type_id) {
							case "head":
								//头图组件 div.plugin -> div.plugin-head-group -> #specialContent
								a ? (s && n.height(s), a = !1, n.appendTo(e.container).append(p)) : (l && s && p.height(s), p.appendTo(e.container));
								break;
							default:
								//其他组件 div.plugin -> #specialContent
								l && s && p.height(s), p.appendTo(e.container)
						}
						//e.pluginHeight -> 已渲染组件的累计高度, e.pluginIndex -> 已渲染组件计数
						e.pluginHeight <= e.preloadHeight && (
							l && (e.pluginHeight += s),
							e.pluginIndex++,
							e.createModule(o, t),
							e.clearDomHeight(p)
					...
					// 处理末尾组件 添加 div#J_notify_tips -> body??
					e.handle Last();
				...
				checkLastModule: function(e) {
					return !(!e.extend || !e.extend.page || "true" != e.extend.page.isPaging) && (e.isLast = !0, !0)
				},
				createModule: function(config, index) {
					if (!config.isLoaded) {//isLoaded -> 标记组件是否已渲染
						var that = this,
							$dom = this.pluginList[index], // $(div.plugin[data-index=n])
						...
						//标记组件已加载
						//html插入组件容器 div.plugin	
						$dom.attr("data-loaded", "true").append(template),
						// 插入css
						"" != css && (css = VIP.modelX(css), //处理model-base
						$dom.append(this.getCssString(css))), //拼接成一个style标签 append 到 div.plugin, jq对dom的操作真是简单，灵活~~

						config.isLast && $dom.append(moreHtml), //组件列表分页加载?
						config.isLoaded = !0;
						
						//执行组件的js eval一个立即执行函数表达式 IIFE
						try {
						...
						// 设置组件内的img.mst-lazy延迟加载
						setTimeout(function() {
							$dom.find("img.mst-lazy").lazyload({
								threshold: that.threshold,
								load: function() {
									this.removeAttribute("height") // <img height="200px" ...>
								}
							})
						}, 50),
				...
				appendCss: function() {
					var e = this,
						// t -> cssList
						t = e.config.data.cssList || [],
						a = "";
					//所有组件的css拼接起来 再用<style>包装， append 到 #specialContent
					t && ($.each(t, function(e, t) {
						a += "\n" + VIP.modelX(t)
					}), e.container.append(e.getCssString(a)))
				},
				bind: function() {
					var e = this;
					//滑动页面时 预加载
				...
				preload: function() {
					var e = this,
						t = doc.body.scrollTop; // t -> body.scrollTop
					e.userScroll = !0, e.container.find(".plugin").each(function(a, i) {
						if (!this.getAttribute("data-loaded")) {
							var n = $(this), //尚未渲染的组件
								o = n.attr("data-index");
							t + doc.documentElement.clientHeight + e.threshold >= n.offset().top && (e.createModule(config.data.moduleList[o], o), e.clearDomHeight(e.pluginList[o]))
						}
					})
				},
				clearDomHeight: function(e, t) {//清除 div.plugin的默认高度， 标记已加载 .plugin-loaded
					var t = t || 1e3;
					setTimeout(function() {
						e.css({
							height: "auto"
				...
				setMars: function() {
					if ("undefined" != typeof window.mstDomStartTime) {
						...
						a.each(function(t, a) {
							var s = $(this),
								d = $(new Image);
							i.push(d), d.attr("src", s.attr("data-original") || s.attr("src")), d.bind("load", function() {
								o++, o == n && (e = (new Date).getTime() - (window.mstDomStartTime || 0) + r, Mar.Seed.request("mst", "click", "mst_firstScreen_time", e)) //所有图片加载完 发送总耗时
							})
						})
					}
				},
				checkLiveTime: function(e) {
					if (!window.config || !window.config.extend) return !0;
					var t = !!window.config.extend.server_time && parseInt(window.config.extend.server_time); // t -> server_time
					if (t && e.model && e.model.live) {
						var a = !!e.model.live.start_time && VIP.stringToDate(e.model.live.start_time),
							...
							n = !!a && a.getTime() / 1e3, // n -> start php timestamp
							o = !!i && i.getTime() / 1e3; // o -> end php timestamp
						return !(n && t < n || o && t > o) //是否在时间段内
					}
					return !0
				},
				initMars: function(e, t, a) {
					var i = window.config;
					//[mst-mars]绑定click事件 发送埋点数据
					i && (VIP.Mars.set(a), ++a, e.on("click", "[mst-mars]", {}, function(i) {
						var n = $(this), // $('[msg-mars]')
							o = $(i.target), // o -> event.target
							r = e.find("[mst-mars]"),
							s = "undefined" == typeof n.attr("mst-mars") ? o.parents("[mst-mars]:eq(0)") : n, // s -> tag[msg-mars]
							d = +r.index(s) + 1, // 序号
							c = +a + "_" + d,
							p = VIP.Mars.get(s.attr("mst-mars"), c),
							l = s.attr("mst-tag"),
						...
						p = $.extend(p, f),
						//发送埋点数据
						Mar.Seed.request(o[0].tagName.toLowerCase(), "click", g, JSON.stringify(p))
					}))
				},
				initEasterEggs: function() {//彩蛋
					VIP.easterEggs && (
						VIP.easterEggs.info.log.data = VIP.defaultParam.log,

						VIP.easterEggs.info.brands.data = $.map(config.data.moduleList, function(e, t) {
							//档期组件
			...
			function() {
				var _at_rule = /@model-base\s*((\d+\/)?\d+)\s*(from\s*(\d+\/)?\d+\s*)?;/i,
					...
					// 若页面的rem标准为 640 1rem = 100;
					// 组件的rem标准为 750 1rem = 75; -> 640 1rem -> 75*640/750 oPxsPerRem -> xRem * oPxsPerRem / nPxsPerRem = 新rem标准下的对应数值
			...
			VIP.cookie = function(e, t, a) {
				...
				var c = a.path ? "; path=" + a.path : "",
					...
					l = a.secure ? "; secure" : "";
				document.cookie = [e, "=", encodeURIComponent(t), s, c, p, l].join("")
			}, 
			...
			VIP.initDefaulParam = function() {
				var e = new Date,
					...
					p = this.getQueryString("app_name"),
					l = this.getQueryString("warehouse") || VIP.getConfigValueByKey("data", "page_data", "warehouse") || "VIP_NH",
				...
				d = /^(phone|pad)$/.test(d) ? "wap" : d, (!window.location.search || "special" == v || parseInt(I) > 1 || !d) && (b = "wap", r = "640", s = "460", d = "wap", c = "4.0", p = "wap", l = this.cookie("warehouse"), u = this.cookie("m_vip_province"), m = "WIFI", f = this.cookie("mars_cid"), g = "1", h = 0, _ = this.cookie("wap_consumer"), y = ""), u = u ? u : 0, m = m ? m : "WIFI", this.defaultParam = {
					verstr: o,
					source: b,
					width: r,
					height: s,
			...
			VIP.getClientImageSrc = function(e, t) {
				...
				if (s = i ? s || "304x384_50" : s || "604x290_50", Array.isArray(s))
					for (var l = 0; l < s.length; l++) switch (o = s[l], o.exp) {
						case "gl:eq:":
			...
			VIP.getToSpecialTopicUrl = function(e) {
				...
				if (e) {
					...
					if (e = i > 0 ? e + "&" + a : e + "?" + a, "wap" == t.client) {
						...
						var c = e.indexOf(o),
							p = e.indexOf("exclusive_subject/te/"),
							l = "http://m.vip.com/index.php?m=special&p=",
							u = "";
						if (c >= 0 && p > 0) {
							...
							u = e.substr(f);
							var g = u.indexOf("?");
							return g > 0 && (u = u.substr(0, g)), "" != u && (e = l + u), e
						}
						return e
					}
					return e
			...
			VIP.link = function() {
				...
				return e.SPECIAL = "http://msp.appvipshop.com/uploadfiles/exclusive_subject/te/v1/s{sid}_{wh}_index.php?wapid=mzt_{sid}&wh={VIP_WH}&title=唯品会", e.SPECIAL_BLANK = "http://msp.appvipshop.com/uploadfiles/exclusive_subject/te/v1/s{sid}_{wh}_index.php?wapid=mzt_{sid}&wh={VIP_WH}&m=webview&title=唯品会", e.PRODUCT = "vipshop://showGoodsDetail?brandId={brand}&goodsId={product}&goodsType=0&goodsTitle={title}&w={VIP_WH}", e.BRAND = "vipshop://showBrandProducts?brandId={brand}&brandName={title}", e.go = function(t, a, i, n, o) {
					...
					n = encodeURIComponent(n), e.VIS = VIP.getToSpecialTopicUrl(a + "&m=webview&title={title}"), d = r(e.VIS), o && "0" !== o.pre_type && (e.BRAND = "vipshop://showBrandProducts?brandId={brand}&brandName={title}&top_type=1&mtms_component_id=" + (o.mtms_component_id || "") + "&mtms_id=" + (window.config.data.build_id || "")), "wap" == p.client && (e.SPECIAL = e.SPECIAL.replace("msp.appvipshop.com", "mzt.vip.com"), e.SPECIAL_BLANK = e.SPECIAL_BLANK.replace("msp.appvipshop.com", "mzt.vip.com"), e.PRODUCT = "http://m.vip.com/product-{brand}-{product}.html?m=product&brand_id={brand}&product_id={product}&title={title}", e.BRAND = "http://m.vip.com/brand-{brand}-0-0-0-1-0-1-40.html?m=brand&brand_id={brand}&title={title}", e.VIS = d ? "http://m.vip.com/index.php?m=special&p=" + d + "&h=mzt.vip.com&_src=vis&title={title}" : e.VIS), l && (e.BRAND = "http://weixin.vip.com/?#productList?brandId={brand}", e.PRODUCT = "http://weixin.vip.com/#productDetail?productId={product}", e.VIS = d ? "http://weixin.vip.com/purchase/special?m=special&p=" + d + "&h=mzt.vip.com&_src=vis&title={title}" : e.VIS);
					var f = "all",
						g = a;
					a && "object" == typeof a && (f = p.warehouse || "nh", g = a[f]);
					var h = null,
						_ = null;
					...
					switch (t) {
						...
						case "app-category":
							...
							break;
						case "index":
							c = l ? "http://weixin.vip.com" : "wap" != p.client ? m ? "vipshop://showMenuItem?typeID=0&typeValue=0" : u && !VIP.compareVersion(p.app_version, "5.1") ? "vipshop://showMenuItem?typeID=0&typeValue=0" : "http://m.vip.com?m=home" : "http://m.vip.com", location.href = c;
							break;
						case "subchannel":
					...
					'<div class="mst-size" id="J-size">' +
					    ...
					    '<div class="mst-size-wrap">' +
					        ...
					        '<div class="mst-size-content">' +
					            '<p class="mst-size-tips"> 尺码 </p>' +
					            '<p class="mst-size-yardage {{if skuList.length <= 2}}mst-size-yardage-big{{/if}}"> {{each(key,value) skuList}} <span data-size-id="${value.id}" {{if type==1 || type==- 1}}class="unactive" {{/if}}>${value.name} <em></em></span> {{/each}} </p>' +
					        '</div>' +
					        '<div class="mst-size-btns"> <a class="mst-size-btn">确 认</a> </div> <span class="mst-size-close" onclick="VIP.Sizing.displayPannel(0)">×</span> </div>' +
					'</div>',
				loading:
					'<div class="mst-global-loading" id="J-mst-global-loading">' +
					    '<div class="mst-gl-mask"></div>' +
					    '<div class="mst-gl-body">' +
					        '<svg class="mst-loading-svg" viewBox="0 0 150 150">' +
					            '<g class="circle">' +
					                '<circle cx="75" cy="75" r="72"></circle>' +
					                '<circle cx="75" cy="75" r="72" class="rotation"></circle>' +
					            '</g>' +
					            '<g class="logo-idle">' +
					                '<path d="M32 62 L47 100 L62 62 M77 49 A0.1 0.1, 0 1 1, 80 52 A0.1 0.1, 0 1 1, 77 49 Z M78 62 L78 100 M95 113 L95 62 L95 85 A15 15, 0 1 1, 130 78 A15 15, 0 1 1, 95 85 Z"></path>' +
					            '</g>' +
					        '</svg>' +
					    '</div>' +
					'</div>'
			...
			VIP.Sizing = {
				...
				pullToApp: function(e, t, a, i, n) {
					...
					switch (n && (this.customCallBack = n), e) {
						case "collect":
							p = n ? "VIP.Sizing.customCallBack" : "VIP.Sizing.pullToCollectCallback", o = "vipshop://addFavourite?sku=" + t + "&goodsId=" + i + "&brandId=" + a + "&isHeat=1&jsFunction=" + p;
							break;
						case "unCollect":
							p = n ? "VIP.Sizing.customCallBack" : "VIP.Sizing.pullToCollectCallback", o = "vipshop://delFavourite?sku=" + t + "&jsFunction=" + p;
							break;
						case "addCart":
							p = n ? "VIP.Sizing.customCallBack" : "VIP.Sizing.pullToCartCallback", o = "vipshop://addCart?sizeId=" + t + "&brandId=" + a + "&productId=" + i + "&sizeNum=1&jsFunction=" + p
					}
				...
				afterInit: function(params) {
					params.position && window.scrollTo(params.position.split(",")[0],
					params.position.split(",")[1]),
					params.action && "" != params.action && eval(params.action + '("' + params.type + '","' + params.sizeId + '","' + params.productId + '")')
				},
			...
			VIP.router = function(e, t, a, i, n) {
				...
				var r = {},
					...
					p = e ? e.getAttribute("mst-tag") : "",
					l = VIP.getQueryString("extra_tag") ? VIP.getQueryString("extra_tag") + "," : "",
					u = p ? "source_tag=" + l + p : "",
					m = l ? l : p,
					f = "all",
				...
				switch (i = encodeURIComponent(i), r = {
					...
					case "subchannel":
						var w = !1,
							...
							P = a[2];
						I && P && (s.is_app && (r.subchannel.app += s.is_android ? "&f=dx" : "", (s.is_iphone && VIP.compareVersion(s.app_version, "5.15") || s.is_android && VIP.compareVersion(s.app_version, "5.7")) && (w = !0, location.href = r.subchannel.app.replace(/{channelID}/g, I).replace(/{channelMenu}/g, P))), w || (s.is_app ? VIP.notify_tips("您当前app版本暂不支持查看本分类频道，建议升级app新版本体验", 3e3) : VIP.notify_tips("本分类商品暂只支持在手机app端查看，请到手机端下载唯品会查看", 3e3)));
						break;
					case "app-category":
						var w = !1,
							a = a.split("|"),
							...
							y = a[1],
							V = $.trim(a[2]);
						b && y && V && ((s.is_iphone || s.is_android) && VIP.compareVersion(s.app_version, "5.14") ? w = !0 : s.isWap && (w = !0), w && (location.href = r.appCategory[c].replace(/{parentId}/g, b).replace(/{categoryId}/g, y).replace(/{categoryName}/g, V))), w || (s.is_app ? VIP.notify_tips("您当前app版本暂不支持查看本分类频道，建议升级app新版本体验", 3e3) : VIP.notify_tips("本分类商品暂只支持在手机app端查看，请到手机端下载唯品会查看", 3e3));
						break;
					case "index":
				...
				set: function(e) {
					...
					t && (++e, t.find("[mst-mars]").each(function(t, a) {
						...
						switch (i.attr("mst-rank", r), o) {
							case "brand":
								n[1] && i.attr("mars_exposure_sead", "special_slide_expose").attr("mars_exposure_module", "mst|brands|" + n[1] + "|" + r + "|0"), d = p + "_" + n[1];
								break;
							case "product":
								n[1] && n[2] && i.attr("mars_exposure_sead", "special_slide_expose").attr("mars_exposure_module", "mst|products|" + n[1] + "|" + r + "|1|" + n[2]), d = p + "_" + n[2];
								break;
							case "url":
								d = p + "_" + VIP.getIdByUrl(n[1]);
								break;
							case "vis":
								d = p + "_" + VIP.getIdByUrl(n[1]);
								break;
			...
			VIP.Favorite = {
				...
				handleDoFav: function(e, t, a) {
					var i = e.data ? e.data[a] || e.data : e.data,
						...
						p = parseInt(d.attr(t.attrPre + "-fav-coupon"));
					if (e.result) e.code ? ("cancel" == i.type ? (n = "收藏成功！", p && (o = VIP.Coupon.handleDoCoupon(s, "", e.brand_id), n = o !== !1 ? n + o : n)) : n = "取消收藏成功", r = "cancel" == i.type ? VIP.Mars.get("blike|" + e.brand_id) : VIP.Mars.get("cblike|" + e.brand_id), Mar.Seed.request("mst", "click", "mst_brand_click", JSON.stringify(r))) : n = "收藏失败!请稍后再试。", n && VIP.notify_tips(n, 2e3), VIP.Favorite.render(t, i.brand_id, c);
					else {
			...
			VIP["interface"] = {
				...
				queryPmsPrice: function(e, t) {
					e.length > 0 ? $.ajax({
						url: "http://mst.vip.com/product/getPmsPriceByProductId",
						dataType: "jsonp",
						data: {
							param: e
						}
						...
						console.log("专享价接口获取失败！")
					}) : console.log("专项价接口参数不正确！")
				}
			}, 
			...
			VIP.Rem = {
				adaption: function() {
					function e() {
						...
						e / d > 540 && (e = 540 * d);
						var t = e / 10;
						o.style.fontSize = t + "px", p.rem = a.rem = t
					}
					var t, a = window,
						...
						d = 0,
						c = 0,
						p = i.flexible || (i.flexible = {});
					if (r) {
						console.warn("将根据已有的meta标签来设置缩放比例");
						var l = r.getAttribute("content").match(/initial\-scale=([\d\.]+)/);
						l && (c = parseFloat(l[1]), d = parseInt(1 / c))
					} else if (s) {
					...
					}, !1), e(), p.dpr = a.dpr = d, p.refreshRem = e, p.rem2px = function(e) {
			...
			win.config && win.config.data && (VIP.easterEggs = {
				count: 0,
				timing: 0,
				info: {
					page_id: {
				...
				timedCount: function() {
					this.count < 9 ? (this.count += 1, this.timing = setTimeout(function() {
			...
			VIP.sliceArr = function(e, t) {
				for (var a = [], t = t || 1, i = 0; i < Math.ceil(e.length / t); i++) {
					var n = i * t,
						o = n + t;
					a.push(e.slice(n, o))
				}
