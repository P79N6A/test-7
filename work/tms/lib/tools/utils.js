...
Utils.headerInfo = function(req, res, next){
	...
	login.getInfo(req, function(info){
		headerInfo = S.merge(headerInfo, info);// 若没有登录的话 headerInfo = {isLogin: false}
		// 超级管理员
...
Utils.token = {
	make: function (uid) {
		return Utils.md5(uid);
	},

	verify: function (token, sessionUid) {
		return token == this.make(sessionUid);
	}
};
