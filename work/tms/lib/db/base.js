...
S.mix(BaseDB.prototype, {
	...
	_add: function(data, callback, errback){
		...
		DBTool.add(_self.config.db, _self.config.col, data, function(err, r){
			if (!err){
				_self.emit('added', {data: r[0]}); // 添加doc成功 触发事件
	...
	// 查询记录
	findBy: function(by, opt, callback, project){
		var _self = this;
		_self._findBy(by, opt, callback, project);
	},
	_findBy: function(by, opt, callback, project){
		var _self = this;
